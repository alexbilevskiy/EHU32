...
void canActionEhuButton1();
void canActionEhuButton2();
...
void twai_init(){
  twai_general_config_t g_config = TWAI_GENERAL_CONFIG_DEFAULT(GPIO_NUM_4, GPIO_NUM_3, TWAI_MODE_NORMAL);         // CAN bus set up
        ...
        if(bt_connected && RxMsg.data[0]==0x0 && CAN_allowDisplay){                     // makes sure "Aux" is displayed, otherwise forward/next buttons will have no effect
          switch(RxMsg.data[1]){
            case 0x81:  DEBUG_PRINT("BUTTON PLAYPAUSE");
                        id(b_play).press();
                        break;
            case 0x91:  DEBUG_PRINT("BUTTON NEXT");                     // upper right button (arrow up)
                        id(b_next).press();
                        break;
            case 0x92:  DEBUG_PRINT("BUTTON PLAYPAUSE");                 // lower right button (arrow down)
                        id(b_prev).press();
                        break;
            default:    break;
          }
